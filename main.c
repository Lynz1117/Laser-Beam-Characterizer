...
#define GAIN_SWITCH_HIGH_V   2.9f
#define GAIN_SWITCH_LOW_V    0.29f
...
#define STARTUP_CLEAR_TARGET_MW       0.20f
...
#define PROFILE_RAW_MAGIC0         0xA5U
#define PROFILE_RAW_MAGIC1         0x5AU
#define PROFILE_RAW_PKT_BEGIN      0x20U
#define PROFILE_RAW_PKT_DATA       0x21U
#define PROFILE_RAW_PKT_END        0x22U
#define PROFILE_RAW_MAX_RECORDS    32U
...
typedef struct
{
    ...
    uint16_t profile_window_buf[ADC_BUF_LEN];
    uint16_t profile_raw_power_buf[PROFILE_CAPTURE_MAX_SAMPLES];
    int32_t  profile_raw_enc_buf[PROFILE_CAPTURE_MAX_SAMPLES];
    uint8_t  profile_raw_stage_buf[PROFILE_CAPTURE_MAX_SAMPLES];
    uint16_t host_repeat_buf_start[PROFILE_REPEAT_BUFFER_MAX_SCANS];
    uint16_t host_repeat_buf_count[PROFILE_REPEAT_BUFFER_MAX_SCANS];
    uint16_t host_repeat_buf_emit_stride[PROFILE_REPEAT_BUFFER_MAX_SCANS];
    int32_t  host_repeat_buf_scan_start_enc[PROFILE_REPEAT_BUFFER_MAX_SCANS];
    int32_t  host_repeat_buf_scan_end_enc[PROFILE_REPEAT_BUFFER_MAX_SCANS];
    int32_t  host_repeat_buf_enc[PROFILE_REPEAT_BUFFER_TOTAL_POINTS];
    uint16_t host_repeat_buf_raw[PROFILE_REPEAT_BUFFER_TOTAL_POINTS];
    uint8_t  host_repeat_buf_stage[PROFILE_REPEAT_BUFFER_TOTAL_POINTS];
} ProfileWorkspace_t;

typedef struct
{
    float    fft_time_buf[FFT_SAMPLES];
    float    fft_out_buf[FFT_SAMPLES];
    float    fft_mag_accum[FFT_SAMPLES / 2U];
    uint16_t fft_chunk_buf[FFT_CHUNK_SIZE];
    uint16_t fft_last_chunk_snap[FFT_CHUNK_HALF];
} FftWorkspace_t;

typedef union
{
    ProfileWorkspace_t profile;
    FftWorkspace_t fft;
} MeasurementWorkspace_t;

static MeasurementWorkspace_t measurement_workspace;

#define profile_power_mw               measurement_workspace.profile.profile_power_mw
#define profile_v_avg                  measurement_workspace.profile.profile_v_avg
#define profile_raw_avg                measurement_workspace.profile.profile_raw_avg
#define profile_stage_avg              measurement_workspace.profile.profile_stage_avg
#define profile_t_ms                   measurement_workspace.profile.profile_t_ms
#define profile_deg                    measurement_workspace.profile.profile_deg
#define profile_window_buf             measurement_workspace.profile.profile_window_buf
#define profile_raw_power_buf          measurement_workspace.profile.profile_raw_power_buf
#define profile_raw_enc_buf            measurement_workspace.profile.profile_raw_enc_buf
#define profile_raw_stage_buf          measurement_workspace.profile.profile_raw_stage_buf
#define host_repeat_buf_start          measurement_workspace.profile.host_repeat_buf_start
#define host_repeat_buf_count          measurement_workspace.profile.host_repeat_buf_count
#define host_repeat_buf_emit_stride    measurement_workspace.profile.host_repeat_buf_emit_stride
#define host_repeat_buf_scan_start_enc measurement_workspace.profile.host_repeat_buf_scan_start_enc
#define host_repeat_buf_scan_end_enc   measurement_workspace.profile.host_repeat_buf_scan_end_enc
#define host_repeat_buf_enc            measurement_workspace.profile.host_repeat_buf_enc
#define host_repeat_buf_raw            measurement_workspace.profile.host_repeat_buf_raw
#define host_repeat_buf_stage          measurement_workspace.profile.host_repeat_buf_stage
#define fft_time_buf                   measurement_workspace.fft.fft_time_buf
#define fft_out_buf                    measurement_workspace.fft.fft_out_buf
#define fft_mag_accum                  measurement_workspace.fft.fft_mag_accum
#define fft_chunk_buf                  measurement_workspace.fft.fft_chunk_buf
#define fft_last_chunk_snap            measurement_workspace.fft.fft_last_chunk_snap
...
volatile uint8_t measurement_abort_flag = 0U;
...
static uint8_t capture_profile_t0_axis(uint8_t axis,
                                       uint32_t *idx_out,
                                       int32_t *t0_enc_out,
                                       int32_t *x0_counts_out,
                                       float *t0_ms_out);
static uint8_t measure_beam_width_for_axis(uint8_t axis, float *w_mm_out);
static void t0_batch_begin_adapter(uint8_t axis, uint8_t num_valid_scans);
static void t0_emit_raw_scan_adapter(uint8_t axis);
...
static void cdc_write_blocking(const uint8_t *buf, uint16_t len)
{
    ...
    CDC_Enqueue_TX_Blocking(buf, len);
    CDC_TX_Kick();
}
...
static void cdc_printf_blocking(const char *fmt, ...)
{
    ...
    CDC_Enqueue_TX_Blocking((const uint8_t *)tx, (uint16_t)n);
    CDC_TX_Kick();
}

static void cdc_wait_for_tx_headroom(uint32_t min_free_bytes, uint32_t timeout_ms)
{
    uint32_t wait_start = HAL_GetTick();

    while (CDC_TX_Free() < min_free_bytes)
    {
        CDC_TX_Kick();
...
static void usb_send_profile_scan_begin_packet(uint8_t axis,
                                               uint8_t scan_no,
                                               uint16_t emit_stride,
                                               ...
                                               uint32_t t0_idx)
{
    ...
    header[0] = PROFILE_RAW_MAGIC0;
    header[1] = PROFILE_RAW_MAGIC1;
    header[2] = PROFILE_RAW_PKT_BEGIN;
    header[3] = axis;
    header[4] = scan_no;
    header[5] = 0U;
    memcpy(&header[6], &emit_stride, sizeof(emit_stride));
    memcpy(&header[8], &record_count, sizeof(record_count));
    memcpy(&header[12], &scan_start_enc, sizeof(scan_start_enc));
    memcpy(&header[16], &scan_end_enc, sizeof(scan_end_enc));
    memcpy(&header[20], &t0_idx, sizeof(t0_idx));

    cdc_write_blocking(header, (uint16_t)sizeof(header));
}

static void usb_send_profile_scan_data_packet(uint8_t axis,
                                              uint8_t scan_no,
...
{
    uint8_t packet[12U + (PROFILE_RAW_MAX_RECORDS * 7U)];
    uint16_t offset = 12U;

    packet[0] = PROFILE_RAW_MAGIC0;
    packet[1] = PROFILE_RAW_MAGIC1;
    packet[2] = PROFILE_RAW_PKT_DATA;
    packet[3] = axis;
    packet[4] = scan_no;
    packet[5] = 0U;
    memcpy(&packet[6], &record_count, sizeof(record_count));
    memcpy(&packet[8], &sample_index_start, sizeof(sample_index_start));

    for (uint16_t i = 0U; i < record_count; i++)
    {
        memcpy(&packet[offset], &enc_buf[i], sizeof(int32_t));
        offset += (uint16_t)sizeof(int32_t);
        memcpy(&packet[offset], &raw_buf[i], sizeof(uint16_t));
        offset += (uint16_t)sizeof(uint16_t);
        packet[offset++] = stage_buf[i];
    }

    cdc_write_blocking(packet, offset);
}

static void usb_send_profile_scan_end_packet(uint8_t axis,
                                             uint8_t scan_no,
                                             uint32_t record_count)
{
    uint8_t header[12];
...
static void usb_emit_profile_scan_from_raw(ProfileAxis_t axis, uint8_t scan_no)
{
    int32_t enc_chunk[PROFILE_RAW_MAX_RECORDS];
    uint16_t raw_chunk[PROFILE_RAW_MAX_RECORDS];
    uint8_t stage_chunk[PROFILE_RAW_MAX_RECORDS];
    uint32_t emitted_count = 0U;

    for (uint32_t i = 0U; i < profile_raw_count; i += t0_dump_emit_stride)
        emitted_count++;

    usb_send_profile_scan_begin_packet((uint8_t)axis,
                                       scan_no,
                                       (uint16_t)t0_dump_emit_stride,
                                       emitted_count,
                                       profiler.scan_start_enc_count,
                                       profiler.scan_end_enc_count,
                                       profiler.t0_idx);

    for (uint32_t base = 0U; base < profile_raw_count; )
    {
        uint16_t chunk_count = 0U;
        uint32_t sample_index_start = base;

        while ((base < profile_raw_count) && (chunk_count < PROFILE_RAW_MAX_RECORDS))
        {
            enc_chunk[chunk_count] = profile_raw_enc_buf[base];
            raw_chunk[chunk_count] = profile_raw_power_buf[base];
            stage_chunk[chunk_count] = profile_raw_stage_buf[base];
            chunk_count++;
            base += t0_dump_emit_stride;
        }

        usb_send_profile_scan_data_packet((uint8_t)axis,
                                          scan_no,
                                          sample_index_start,
                                          enc_chunk,
                                          raw_chunk,
    ...
    usb_send_profile_scan_end_packet((uint8_t)axis, scan_no, emitted_count);
}

static void usb_emit_profile_scan_from_buffered(uint8_t axis,
                                                uint8_t scan_no,
                                                uint16_t start,
                                                uint16_t count,
                                                uint16_t emit_stride,
                                                ...
                                                int32_t scan_end_enc)
{
    ...
    usb_send_profile_scan_begin_packet(axis,
                                       scan_no,
                                       emit_stride,
                                       count,
                                       scan_start_enc,
    ...
    {
        uint16_t chunk_count = 0U;
        uint32_t sample_index_start = (uint32_t)emitted * (uint32_t)emit_stride;

        while ((emitted < count) && (chunk_count < PROFILE_RAW_MAX_RECORDS))
        {
            uint16_t slot = (uint16_t)(start + emitted);
            enc_chunk[chunk_count] = host_repeat_buf_enc[slot];
            raw_chunk[chunk_count] = host_repeat_buf_raw[slot];
            stage_chunk[chunk_count] = host_repeat_buf_stage[slot];
            chunk_count++;
            emitted++;
        }

        usb_send_profile_scan_data_packet(axis,
                                          scan_no,
                                          sample_index_start,
                                          enc_chunk,
                                          raw_chunk,
    ...
    usb_send_profile_scan_end_packet(axis, scan_no, count);
}

static void t0_emit_buffered_scans_and_results(ProfileAxis_t axis, uint8_t valid_scans)
{
    // Emit the per-scan T0 metadata before the larger buffered raw dump so the
    // state machine does not need to push these small control lines through a
    // nearly full CDC queue after the binary stream finishes.
    for (uint8_t scan = 0U; scan < valid_scans; scan++)
    {
        cdc_printf_blocking("T0_RESULT,axis=%u,scan=%u,idx=%lu,t0_enc=%ld,x0_counts=%ld,t0_ms=%.6f\r\n",
                            (unsigned int)axis,
                            (unsigned int)scan,
                            (unsigned long)t0_result_idx[scan],
                            (long)t0_result_enc[scan],
                            (long)t0_result_x0_counts[scan],
                            t0_result_ms[scan]);
    }

    host_emit_buffered_repeat_scans((uint8_t)axis);
}
// -----------------------------
...
static void do_stokes_scan(void)
{
    const StokesMeasurementContext ctx = {
        .num_samples = NUM_STOKES_SAMPLES,
        .steps_per_increment = STEPS_PER_INCREMENT,
        .settle_ms = SETTLE_MS,
        .abort_flag = &measurement_abort_flag,
        .samples_raw = stokes_I,
        .samples_v = stokes_V,
        .samples_mw = stokes_mW,
        .samples_stage = stokes_stage,
        .stepper_wake = stepper_sleep_wake,
        .set_dir = drv_set_dir,
        .delay_ms = HAL_Delay,
        .move_steps = move_steps,
        .capture_autorange = capture_adc_autorange,
        .adc_counts_to_voltage = adc_counts_to_voltage,
        .voltage_to_power_mw = voltage_to_power_mw,
        .log_printf = printf
    };

    Measurement_Stokes_Run(&ctx);
#if 0
...
static void t0_batch_begin(ProfileAxis_t axis, uint8_t num_valid_scans)
{
    ...
    cdc_printf_blocking("T0_BATCH_BEGIN,axis=%u,num_scans=%u\r\n",
...
static uint8_t t0_dump_scan_step(ProfileAxis_t axis, uint8_t scan_no)
{
    ...
    if (!t0_dump_started)
    {
        ...
        t0_dump_idx         = 0U;
        t0_dump_win_end     = n;
        t0_dump_emit_stride = 4U;
        t0_dump_started     = 1U;

        cdc_wait_for_tx_headroom(1024U, 500U);
        CDC_TX_Kick();
        HAL_Delay(2);

        cdc_printf_blocking("RAW_SCAN_BEGIN,axis=%u,scan=%u,stride=%lu,emit_stride=1,"
    ...
    while ((t0_dump_idx < t0_dump_win_end) && (lines_this_call < max_lines_per_call))
    {
        ...
        cdc_printf_blocking("RAW_SCAN,%u,%u,%lu,%ld,%.6f,%.6f,%u,%u,%.6f,%.6f\r\n",
                            (unsigned int)axis,
                            (unsigned int)scan_no,
                            (unsigned long)i,
                            (long)enc,
    ...
    {
    	uint32_t emitted_n = (t0_dump_win_end + t0_dump_emit_stride - 1U) / t0_dump_emit_stride;

        cdc_wait_for_tx_headroom(512U, 500U);
        CDC_TX_Kick();

        cdc_printf_blocking("RAW_SCAN_DONE,axis=%u,scan=%u,n=%lu,full_window=1\r\n",
                            (unsigned int)axis,
                            (unsigned int)scan_no,
                            (unsigned long)emitted_n);
...
static void t0_batch_update(void)
{
    ...
    if (measurement_abort_flag)
    {
        printf("T0_BATCH_ABORT,axis=%u,scan=%u\r\n",
               (unsigned int)t0_batch_axis,
    ...
    switch (t0_batch_state)
    {
        ...
        case T0_BATCH_WAIT_STABLE:
        {
            ...
            cdc_printf_blocking("T0_SCAN_BEGIN,axis=%u,scan=%u,counts=%d\r\n",
                                (unsigned int)t0_batch_axis,
                                (unsigned int)t0_batch_scan_index,
...
//        break;
        ...
        case T0_BATCH_COMPUTE:
        {
            ...
            if (!host_buffer_current_repeat_scan(t0_batch_valid_scans))
            {
                printf("PROFILE_REPEAT_BUFFER_OVERFLOW,axis=%u,scan=%u,n=%lu\r\n",
                       (unsigned int)t0_batch_axis,
                       (unsigned int)t0_batch_valid_scans,
                       (unsigned long)profile_raw_count);
        ...
        // --------------------------------------------------
        // Legacy immediate dump state left inert; valid scans are buffered
        // while the motor is running and emitted after the axis is cleared.
        // --------------------------------------------------
        case T0_BATCH_DUMP:
            t0_batch_state = T0_BATCH_NEXT;
            break;
...
//        break;
        ...
        case T0_BATCH_CLEAR_BEAM:
            ...
            dc_motors_stop_all();
            t0_emit_buffered_scans_and_results((ProfileAxis_t)t0_batch_axis,
                                               t0_batch_valid_scans);
        ...
        {
            cdc_printf_blocking("T0_BATCH_AXIS_DONE,axis=%u\r\n",
                                (unsigned int)t0_batch_axis);

            // Let the previous axis dump drain enough that the next axis
            // control messages and scan startup are not lost behind a full CDC
            // queue.
            cdc_wait_for_tx_headroom(3072U, 2000U);
            CDC_TX_Kick();

            // Reuse the same known-good batch setup path as axis 0 so the
            // second axis gets identical motor spin-up, encoder reset, and
            // WAIT_STABLE startup behavior.
            dc_motors_stop_all();
            t0_batch_begin(PROFILE_AXIS_M2, t0_batch_num_scans);
        }
        break;
...
#if 0
// Legacy inlined divergence and M2 implementations retained for reference
// while the active workflows live in dedicated measurement modules.
...
static void t0_emit_raw_scan(ProfileAxis_t axis)
{
    // Single-axis T0 dumps happen after the motor is stopped, so send one
    // compact binary stream instead of pacing thousands of text lines.
    t0_dump_started = 0U;
    t0_dump_idx = 0U;
    t0_dump_emit_stride = 4U;
    usb_emit_profile_scan_from_raw(axis, 0U);
    live_power_update_service();
}
...
#if 0
// Legacy M2-specific analysis and scan runners retained for reference.
...
static uint8_t capture_profile_t0_axis(uint8_t axis,
                                       ...
                                       float *t0_ms_out)
{
    profiler.axis = (ProfileAxis_t)axis;
    profile_find_t0_only((ProfileAxis_t)axis);

    if (!host_local_t0_ready)
        return 0U;

    if (idx_out != NULL)
        *idx_out = profiler.t0_idx;
    if (t0_enc_out != NULL)
        *t0_enc_out = profiler.t0_enc_count;
    if (x0_counts_out != NULL)
        *x0_counts_out = profiler.x0_counts;
    if (t0_ms_out != NULL)
        *t0_ms_out = profiler.t0_s * 1000.0f;
...
{
    profiler.axis = (ProfileAxis_t)axis;
    return measure_beam_width_d4sigma(w_mm_out);
}

static void t0_batch_begin_adapter(uint8_t axis, uint8_t num_valid_scans)
{
    t0_batch_begin((ProfileAxis_t)axis, num_valid_scans);
}

static void t0_emit_raw_scan_adapter(uint8_t axis)
{
    t0_emit_raw_scan((ProfileAxis_t)axis);
}

static uint8_t run_divergence_measurement(float step_mm, float total_mm)
{
    const DivergenceMeasurementContext ctx = {
        .busy_flag = &divergence_busy,
        .abort_flag = &measurement_abort_flag,
        .t0_batch_active = &t0_batch_active,
        .t0_batch_total_axes = &t0_batch_total_axes,
        .z_current_mm = &z_current_mm,
        .z_steps_per_mm = Z_STEPS_PER_MM,
        .z_display_scale = 2.0f,
        .z_settle_ms = Z_SETTLE_MS,
        .z_home_dir = Z_HOME_DIR,
        .z_away_dir = Z_AWAY_DIR,
        .z_home_simple = z_home_simple,
        .home_pressed = home_pressed,
        .z_stepper_wake = z_stepper_wake,
        .z_stepper_disable = z_stepper_disable,
        .z_set_dir = z_set_dir,
        .z_move_steps = z_move_steps,
        .z_move_mm_dir = z_move_mm_dir,
        .delay_ms = HAL_Delay,
        .t0_batch_begin = t0_batch_begin_adapter,
        .t0_batch_update = t0_batch_update,
        .live_power_update_service = live_power_update_service,
        .capture_axis_t0 = capture_profile_t0_axis,
        .clear_active_axis_out_of_beam = clear_active_axis_out_of_beam,
        .emit_raw_scan = t0_emit_raw_scan_adapter,
        .log_printf = printf
    };

    // Centralize the multi-position divergence scan in its own module while reusing shared hardware services.
    return Measurement_Divergence_Run(&ctx, step_mm, total_mm);
}
...
static uint8_t run_step_divergence(float step_mm, uint32_t num_positions)
{
    ...
    return Measurement_Divergence_RunStepScan(&ctx, step_mm, num_positions);
}

static uint8_t run_m2_measurement(float step_mm, float total_mm)
{
    const M2MeasurementContext ctx = {
        .busy_flag = &divergence_busy,
        .abort_flag = &measurement_abort_flag,
        .z_current_mm = &z_current_mm,
        .z_settle_ms = Z_SETTLE_MS,
        .z_home_dir = Z_HOME_DIR,
        .z_away_dir = Z_AWAY_DIR,
        .travel_max_mm = M2_TOTAL_MM,
        .z_home_simple = z_home_simple,
        .home_pressed = home_pressed,
        .z_move_mm_dir = z_move_mm_dir,
        .z_move_to_mm = z_move_to_mm,
        .delay_ms = HAL_Delay,
        .capture_axis_t0 = capture_profile_t0_axis,
        .measure_axis_width = measure_beam_width_for_axis,
        .log_printf = printf
    };

    return Measurement_M2_Run(&ctx, step_mm, total_mm);
}
...
static uint8_t run_m2_capture_at_position(float target_mm, uint32_t step_index)
{
    ...
    return Measurement_M2_CaptureAtPosition(&ctx, target_mm, step_index);
}
...
//    profile_extract_latest_window(win_buf, nsamp);
    ...
    profile_extract_window_from_index(profile_window_buf, profiler.window_start_idx, nsamp);

    // Convert ADC → power with gain handling
    profile_update_power_buffer_from_adc_window(profile_window_buf, nsamp);
...
static void host_emit_buffered_repeat_scans(uint8_t axis)
{
    for (uint8_t scan_slot = 0U; scan_slot < host_repeat_buf_valid_scans; scan_slot++)
    {
        ...
        // Emit buffered scans as compact binary records once the profile motor
        // is out of the timing-critical capture phase.
        usb_emit_profile_scan_from_buffered(axis,
                                            scan_no,
                                            start,
                                            count,
                                            emit_stride,
        ...
        if (scan_no == 0U)
        {
            cdc_printf_blocking("INIT_SCAN_READY,axis=%u,n=%u\r\n",
                                (unsigned int)axis,
                                (unsigned int)count);
        }
        else
        {
            cdc_printf_blocking("REPEAT_SCAN_READY,axis=%u,scan=%u,n=%u\r\n",
                                (unsigned int)axis,
                                (unsigned int)scan_no,
                                (unsigned int)count);
        }
                                                              ...
                                                              uint32_t max_n)
{
    ...
    for (uint32_t i = 0; i < nsamp; i++)
    {
        raw_dst[i] = profile_window_buf[i];
...
static void do_wavelength_fft_measurement(void)
{
    uint16_t *fft_chunk_buf_ptr = fft_chunk_buf;
    WavelengthMeasurementContext ctx = {
        .hadc = &hadc1,
        .sample_timer = &htim6,
        .fft_busy = &fft_busy,
        .fft_abort_flag = &fft_abort_flag,
        .fft_chunk_half_ready = &fft_chunk_half_ready,
        .fft_chunk_full_ready = &fft_chunk_full_ready,
        .chunk_buf = fft_chunk_buf_ptr,
        .fft_chunk_size = FFT_CHUNK_SIZE,
        .fft_chunk_half = FFT_CHUNK_HALF,
        .fft_adc_fs_hz = FFT_ADC_FS_HZ,
        .wl_window_samples = WL_WINDOW_SAMPLES,
        .wl_window_time_s = WL_WINDOW_TIME_S,
        .step3_speed_mm_s = STEP3_SPEED_MM_S,
        .wl_num_runs = WL_NUM_RUNS,
        .stepper_wake = stepper_sleep_wake,
        .stepper_sleep_disable = stepper_sleep_disable,
        .set_dir = drv2_set_dir,
        .step_pulse = step2_pulse,
        .delay_us = delay_us,
        .delay_ms = HAL_Delay,
        .step_period_us = step3_step_period_us,
        .get_tick = HAL_GetTick,
        .enqueue_blocking = CDC_Enqueue_TX_Blocking,
        .tx_kick = CDC_TX_Kick,
        .log_printf = printf
    };

    Measurement_Wavelength_Run(&ctx);
}
...
//   ABORT → stop active measurement / profiling
// -----------------------------
void process_cmd_byte(uint8_t b)
{
    ...
    if (b == '\n')
    {
        ...
        {
            printf("ABORT CMD RECEIVED\r\n");
            measurement_abort_flag = 1U;
            fft_abort_flag = 1U;
...
static void clear_beam_both_motors_startup(void)
{
    ...
    printf("STARTUP_CLEAR_BOTH_BEGIN,target_mw=%.4f\r\n",
           STARTUP_CLEAR_TARGET_MW);

    // Spin both blades together and wait until the live power stays high,
    // which indicates neither blade is cutting into the beam path anymore.
    dc_motors_stop_all();
    HAL_Delay(50);
    ...
    while ((HAL_GetTick() - start_ms) < PROFILE_CLEAR_TIMEOUT_MS)
    {
        ...
        if ((last_log_ms == 0U) || ((now - last_log_ms) >= 100U))
        {
            printf("STARTUP_CLEAR_PWR,p=%.4f,confirm=%u\r\n",
                   p,
                   (unsigned int)clear_count);
            last_log_ms = now;
        }

        if (p >= STARTUP_CLEAR_TARGET_MW)
        {
            if (clear_count < PROFILE_CLEAR_CONFIRM_COUNT)
                clear_count++;
        }
        else
        {
        ...
        {
            dc_motors_stop_all();
            printf("STARTUP_CLEAR_SUCCESS,p=%.4f,confirm=%u\r\n",
                   p,
                   (unsigned int)clear_count);
    ...
    dc_motors_stop_all();
    printf("STARTUP_CLEAR_TIMEOUT,p=%.4f,target_mw=%.4f\r\n",
           latest_p_mw,
           STARTUP_CLEAR_TARGET_MW);
    printf("STARTUP_CLEAR_BOTH_DONE\r\n");
}
